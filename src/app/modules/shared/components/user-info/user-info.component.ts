...
export class UserInfoComponent implements OnInit , OnDestroy {
	api = environment.api || null;
	currentUser = {
		avatar  : '../../../../assets/images/avatar/default_avatar.png' ,
		email   : 'Email' ,
		name    : 'User name' ,
		role    : 'user role'
	};
	...
	confirmMessage() {
		this.confirmService.confirmMessage('Confirmation', 'Are you sure?', 'Yes', 'No', () => {
		  console.log('Accepted');
		  localStorage.removeItem("AbLPDaGK");
		  localStorage.removeItem("ZpeJk7zV");
		  window.location.href = '/login';
		});
	  }
	async signOut() {
		this.confirmService.confirmDelete('Cảnh báo ⚠️', 'Bạn chắc chắn muốn đăng xuất?', 'Có', 'Hủy', () => {
			console.log('Accepted');
			localStorage.removeItem("AbLPDaGK");
			localStorage.removeItem("ZpeJk7zV");
			
			// Thực hiện các hành động cần thiết khác nếu có
			this.router.navigate(['/login']); // Redirect đến trang login
		});
